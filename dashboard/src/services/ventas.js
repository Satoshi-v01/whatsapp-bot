import api from './api'

export async function getVentas() {
    const res = await api.get('/ventas')
    return res.data
}

export async function getVentasPorEstado(estado) {
    const res = await api.get(`/ventas/estado/${estado}`)
    return res.data
}

export async function actualizarEstadoVenta(id, estado) {
    const res = await api.patch(`/ventas/${id}/estado`, { estado })
    return res.data
}

export async function actualizarMetodoPago(id, metodo_pago, cuenta_transferencia_id = null, subtipo_pago = null) {
    const res = await api.patch(`/ventas/${id}/metodo-pago`, { metodo_pago, cuenta_transferencia_id, subtipo_pago })
    return res.data
}

export async function anularVenta(id) {
    const res = await api.post(`/ventas/${id}/anular`)
    return res.data
}

export async function registrarVentaPresencial(datos) {
    const res = await api.post('/ventas/presencial', datos)
    return res.data
}

export async function getHistorial(params = {}) {
    const query = new URLSearchParams(params).toString()
    const res = await api.get(`/ventas/historial${query ? `?${query}` : ''}`)
    return res.data
}

export async function getReporte(params = {}) {
    const query = new URLSearchParams(params).toString()
    const res = await api.get(`/ventas/reporte/exportar${query ? `?${query}` : ''}`)
    return res.data
}

export async function getLibroVentas(fecha_desde, fecha_hasta) {
    const res = await api.get(`/ventas/libro-ventas?fecha_desde=${fecha_desde}&fecha_hasta=${fecha_hasta}`)
    return res.data
}